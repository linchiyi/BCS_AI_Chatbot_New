...
LAB_DATA: Dict[str, Any] = {
    "CBC（血液常規）": {
        "WBC（白血球）": "18,200 /μL（Seg.: 79.8%, Band.: 4%）",
        "Hb（血紅素）": "13.1 g/dL",
        "PLT（血小板）": "96,000 /μL",
    },
    "SMAC（生化檢驗）": {
    ...
    "腹水檢驗（Ascites Analysis）": {
        "Protein": "215 mg/dL",
        "Glucose": "12 mg/dL",
        "LDH": "249 U/L",
        "Cell count - WBC": "12,000 /μL",
        "Cell count - RBC": "200 /μL",
    },
}
...
# =========================================================
CT_IMAGES: List[str] = [
    "截圖 2026-01-14 上午9.56.34.png",
    "截圖 2026-01-14 上午9.56.52.png",
]
...
- 【時機控制】你一開始不會主動提轉院。只有當醫學生明確說到「手術」「開刀」「要開」這類字眼後，你才會在下一個回覆中提出轉院的想法。
...
- **【動態情緒強度】** 每次回覆的最後，請根據這次對話的內容評估當前情緒強度（1-5分），並用以下格式附加在回覆最後：`{{EMOTION:X}}`，其中 X 是 1-5 的數字。
  **判斷標準：**
  - 1分【平靜/配合】：日常問答、理解說明、配合對話。例：「好，我知道了。」
  - 2分【擔心/不安】：開始追問細節、表達擔憂、略顯焦慮。例：「這樣會不會有危險？」
  - 3分【焦慮/明顯情緒】：情緒明顯波動、開始擔心後果、難以專注。例：「所以我爸爸一定要開刀？」
  - 4分【強烈情緒】：焦急/恐懼/憤怒明顯、開始質疑或堅持其他方案。例：「那要不要轉到大醫院？」
  - 5分【情緒崩潰/極端反應】：哭泣/自責/完全無法接受、需要時間冷靜。例：「都是我的錯，我沒有照顧好他......」
  **動態調整原則：**
  - 醫學生同理、明確說明後，情緒可下降 1-2 分
  - 聽到壞消息細節（如手術風險、不開刀後果）後，情緒上升 1-2 分
  - 同一情緒模式下，強度仍可隨對話波動
""".strip()
